...
class dbStatement
{
    ...
    public function __construct($dbh, $statement, $sql)
    {
	$this->dbh = $dbh;
	$this->sql = $sql;
    ...
    {
	/* Get cache. */
        $key = 'fetch-' . $this->cursorOffset . '-' . md5($this->sql);

	$res = $this->dbh->getCache($key, $this->sql);
	if($res != DAO::CACHE_MISS)
	{
	    $this->cursorOffset++;
            return $res;
	}

        $result = $this->pdoStatement->fetch($mode, $cursorOrientation, $this->cursorOffset);
	$this->cursorOffset++;
	$this->dbh->setCache($key, $result);
...
class dbh
{
	...
	if($config->cache->dao->enable)
        {
            try
            {
                $this->daoCache = new cache($config->cache->dao->driver, $config->db->name . '-dao-', $config->cache->dao->lifetime);
            }
            catch (Exception $e)
            {
                die($e->getMessage());
	...
	/* Flush cache. */
	/*
	preg_match_all("/({$config->db->prefix}\w+)[`\" ]/", $sql, $tables);
	if(count($tables) > 0)
	{
	    $table = $tables[1][0];
	    if($table) $this->setCache($table);
	}
	 */
    ...
    {
        if(empty($this->daoCache)) return false;
        $this->daoCache->set($key, array(microtime(true), $value));
    }
    ...
    {
        $newSql = $this->formatSQL($sql);
        if(class_exists('dao')) dao::$querys[] = "[$this->flag] " . dao::processKeywords($newSql);

        //$statement = $this->pdo->query($newSql);
	//return new dbStatement($this, $statement, $sql);
        return $this->pdo->query($newSql);
    }

    public function getCache($key, $sql)
    {
        if(empty($this->daoCache)) return self::CACHE_MISS;

        $cache = $this->daoCache->get($key);
        ...
        {
            $tableCache = $this->daoCache->get($table);
