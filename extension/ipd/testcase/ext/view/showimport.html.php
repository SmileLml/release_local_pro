...
$('#showData').on('change', '.picker-select', function(e)
{
    ...
    if(field === 'module') setStories(index, $(this).val());
});
...
function setStories(index, moduleID = '', branchID = '')
{
    var moduleID   = moduleID == '' && $('#module' + index).val() == '' ? 0 : $('#module' + index).val();
    var branchID   = branchID == '' && $('#branch' + index).val() == '' ? 0 : $('#branch' + index).val();
    var stroyValue = $('#story' + index).val() == '' ? 0 : $('#story' + index).val();
    var storyLink  = createLink('story', 'ajaxGetProductStories', 'productID=' + productID + '&branch=' + branchID + '&moduleID=' + moduleID + '&storyID=' + stroyValue + '&onlyOption=false&status=&limit=0&type=full&hasParent=1&executionID=0&number=' + index);
