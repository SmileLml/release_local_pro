...
include $app->getModulePath() . 'view/datatable.fix.html.php';
...
js::set('batchCheckMax',    $config->testcase->batchCheckMax);
js::set('batchCheckMaxLang',$lang->testcase->batchCheckMax);
$canBeChangedProject = true;
if($this->app->tab == 'project' && isset($project) && !common::canModify('project', $project)) $canBeChangedProject = false;
if($this->app->tab != 'project' && !common::canModify('projectproduct', $product)) $canBeChangedProject = false;
?>
  ...
  <div class='main-col'>
      ...
      <div class="table-empty-tip">
        <p>
          ...
          else
          {
            ...
            if((empty($productID) || common::canModify('product', $product)) && common::hasPriv('testcase', 'create') && $browseType != 'bysuite' && $canBeChangedProject)
            {
              $initModule = isset($moduleID) ? (int)$moduleID : 0;
    ...
    <?php else:?>
      ...
      <form class='main-table table-case' data-nested='true' data-expand-nest-child='false' data-checkable='true' data-enable-empty-nested-row='true' data-replace-id='caseTableList' data-preserve-nested='true'
        ...
        $canIgnoreLibcaseChange     = common::hasPriv('testcase', 'ignoreLibcaseChange');
        $canBatchAction             = ($canBatchRun || $canBatchEdit || $canBatchDelete || $canBatchCaseTypeChange || $canBatchConfirmStoryChange || $canBatchChangeModule || $canImportToLib || $canConfirmLibcaseChange || $canIgnoreLibcaseChange);
        ?>
          ...
          <div class='table-actions btn-toolbar'>
            ...
            <div class="btn-group dropup">
              ...
              <div class="btn-group dropup">
                ...
                <?php endif;?>
                <?php
                if($canImportToLib and $canBeChangedProject)
                {
                ...
                <div class="btn-group dropup">
                  ...
                  <?php
                  if($canConfirmLibcaseChange and $canBeChangedProject)
                  {
                    $actionLink = $this->createLink('testcase', 'batchConfirmCaseUpdate', 'taskID=0&confirm=no');
                    $misc       = "onclick=\"setFormActionCustom('$actionLink', '', '#caseList')\"";
                    echo html::a('#', $lang->testcase->batchConfirmCaseUpdate, 'hiddenwin', "class='btn btn-primary' $misc");
                  }
                  if($canIgnoreLibcaseChange and $canBeChangedProject)
                  {
                    $actionLink = $this->createLink('testcase', 'batchIgnoreCaseUpdate', 'confirm=no');
                    $misc       = "onclick=\"setFormActionCustom('$actionLink', '', '#caseList')\"";
                    echo html::a('#', $lang->testcase->batchIgnoreCaseUpdate, 'hiddenwin', "class='btn btn-primary' $misc");
                  }
                  ?>
                </div>
