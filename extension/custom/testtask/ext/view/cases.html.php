...
<?php js::set('batchCheckMax',    $config->testcase->batchCheckMax); ?>
<?php js::set('batchCheckMaxLang',$lang->testcase->batchCheckMax);?>
<?php
  $canBeChangedByProject = true;
  if(isset($task->projectStatus) && isset($config->CRProject) && empty($config->CRProject) && $task->projectStatus == 'closed') $canBeChangedByProject = false;
?>
<div id='mainContent' class='main-row fade'>
  ...
  <div class='main-col'>
    ...
    id='caseForm' method='post' <?php if(!$useDatatable) echo "data-ride='table'";?>>
      ...
      $canbatchConfirmchange = common::hasPriv('testcase', 'confirmchange');

      $canBatchAction = ($canBeChanged and $canBeChangedByProject and ($canBatchEdit or $canBatchUnlink or $canBatchAssign or $canBatchRun or $canbatchConfirmchange));
      ...
      array_splice($config->testcase->datatable->defaultField, 3, 0, 'caseUpdateMark');
      ...
      <?php if(!$useDatatable) echo '<div class="table-responsive">';?>
        <table class='table has-sort-head table-fixed table-nested table has-sort-head<?php if($useDatatable) echo ' datatable';?>' id='caseList' data-fixed-left-width='<?php echo $widths['leftWidth']?>' data-fixed-right-width='<?php echo $widths['rightWidth']?>' data-checkbox-name='caseIDList[]'>
          <thead>
            ...
            {
              if($value->id == 'actions' && !$canBeChangedByProject)
              {
                unset($setting[$key]);
                continue;
              }
      ...
      <div class='table-footer'>
        ...
        <div class='table-actions btn-toolbar'>
          ...
          {
            $actionLink = $this->createLink('testcase', 'batchConfirmCaseUpdate', "taskID=$taskID");
            $misc       = "onclick=\"setFormActionCustom('$actionLink')\"";
      ...
      <div class="table-empty-tip">
        <p><span class="text-muted"><?php echo $lang->testcase->noCase;?></span> <?php if($canBeChanged && $canBeChangedByProject) common::printLink('testtask', 'linkCase', "taskID={$taskID}", "<i class='icon icon-link'></i> " . $lang->testtask->linkCase, '', "class='btn btn-info'");?></p>
      </div>
      <?php endif;?>
    </form>
  </div>
