    ...
    <?php if(!isonlybody()):?>
      <div class="btn-toolbar pull-right">
        <?php
          if($libType == 'api')
          {
            if(common::hasPriv('api', 'struct') and $canBeModify)        echo html::a($this->createLink('api', 'struct',        "libID=$libID"), "<i class='icon-treemap muted'> </i>" . $lang->api->struct, '', "class='btn btn-link'");
            if(common::hasPriv('api', 'releases') and $canBeModify)      echo html::a($this->createLink('api', 'releases',      "libID=$libID", 'html', true), "<i class='icon-version muted'> </i>" . $lang->api->releases, '', "class='btn btn-link iframe' data-width='800px'");
            if(common::hasPriv('api', 'createRelease') and $canBeModify) echo html::a($this->createLink('api', 'createRelease', "libID=$libID"), "<i class='icon-publish muted'> </i>" . $lang->api->createRelease, '', "class='btn btn-link iframe' data-width='800px'");
          }
          ...
          if(common::hasPriv('doc', 'createLib') and $canBeModify)
          {
          ...
          if($libType == 'api')
          {
            if(common::hasPriv('api', 'create') and $canBeModify) echo html::a($this->createLink('api', 'create',    "libID=$libID&moduleID=$moduleID", '', true), '<i class="icon icon-plus"></i> ' . $lang->api->createApi, '', 'class="btn btn-primary iframe" data-width="95%"');
          }
          elseif($libID and common::hasPriv('doc', 'create') and $canBeModify)
          {
