...
<?php
$canBeModify = true;
if($type == 'execution') $canBeModify = common::canModify('execution', $object);
if($type == 'project')
{
  if(!empty($doc->execution)) $canBeModify = common::canModify('execution', $object);
  else $canBeModify = common::canModify('project', $object);
}
js::set('canBeModify', $canBeModify ? 1 : 0);
?>
  ...
  <?php if(empty($object->deleted)):?>
    <div class="btn-toolbar pull-right">
      <?php
        ...
        if(common::hasPriv('doc', 'createLib') and $canBeModify) echo html::a($this->createLink('doc', 'createLib', "type=" . ($objectType ? $objectType : 'nolink') . "&objectID=$objectID"), '<i class="icon icon-plus"></i> ' . $lang->api->createLib, '', 'class="btn btn-secondary iframe" data-width="800px"');
        if(common::hasPriv('doc', 'create') and $canBeModify) echo $this->doc->printCreateBtn($lib, $moduleID);
      ?>
    </div>
  <?php endif;?>
</div>
  ...
  <?php else:?>
    <div class="cell">
      <div class="detail empty text-center">
        ...
        <?php if($canBeModify) echo html::a($this->createLink('doc', 'createLib', "type={$objectType}&objectID=$object->id"), "<i class='icon icon-plus'></i> " . $lang->doc->createLib, '', "class='btn btn-info iframe'");?>
      </div>
    </div>
  <?php endif;?>
</div>
