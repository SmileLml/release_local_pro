...
class company extends control
{
    public function calendar($parent = '', $begin = 0, $end = 0, $product = 0, $project = 0, $execution = 0, $userID = '', $showUser = 'logged', $iframe = 'no', $userType = '')
    {
        ...
        if($iframe == 'yes')
        {
            ...
            $this->view->datas     = $this->company->getEffortCustom($parent, $begin, $end, $product, $project, $execution, $account, $showUser, $userType);
            $this->view->users     = $this->loadModel('user')->getPairsCustom();
        ...
        /* $endStrtoTime   = $end == 0 ? strtotime('now') : strtotime($end);
        if(date('m-d', $endStrtoTime) == '02-29') $beginStrtoTime = $begin == 0 ? strtotime('-1 year -1 day') : strtotime($begin);
        else $beginStrtoTime = $begin == 0 ? strtotime('-1 year') : strtotime($begin);

        $end   = date('Y-m-d', $endStrtoTime);
        $begin = date('Y-m-d', $beginStrtoTime); */
        ...
        /* Get users.*/
        $users      = array('' => '') + $this->loadModel('dept')->getDeptUserPairs($parent, 'id', '', 'withdeleted');
        $usersCount = count($users);
        if($usersCount > $this->config->maxCount) $this->config->maxCount = ++$usersCount;
