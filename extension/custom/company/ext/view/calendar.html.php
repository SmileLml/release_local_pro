...
<?php js::set('deleteLang', $lang->user->dimission);?>
          ...
          <div class='detail-content'>
            ...
            <div class='form-group'>
              <div id='userBox' class='input-group'>
                ...
                <?php echo html::select('user', $users, $userID, 'class="form-control chosen" data-option-template="<li class=\'option-template\' ><span>__country_flag__</span></li>'.'" ');?>
              </div>
            </div>
            <div class='form-group'>
              <div class='input-group'>
...
$(function()
{
  ...
  $("#userBox #user").chosen().on('chosen:showing_dropdown', function(e){ replaceDeleteTag();});
  $('#user_chosen .chosen-search input').on('keyup', function(e){ replaceDeleteTag();});
  function replaceDeleteTag()
  {
    $('#user_chosen').find('li').each(function(index, element)
    {
      var str = $(element).attr('title');
      if(str != undefined)
      {
        str     = str.replace("(deleted)", "");
        if(str != $(element).attr('title')) $(element).html('<em></em>' + str + '&nbsp;<span class="label label-danger">' + deleteLang + '</span>');
      }
    });
  }
})
