...
if($from == 'project' and !empty($projectID)) $canBeChangedByProject = common::canModify('project', $project);
js::set('canBeChangedByProject', $canBeChangedByProject);
$hasChildren = false;
array_map(function($story) use(&$hasChildren, $canBeChangedByProject)
{
...
if(!$canBeChangedByProject)
{
  ...
  {
    if($colValue->name == 'actions' and $colKey > 0 )
    {
      unset($cols[$colKey]);
      $cols[$colKey - 1]->width = 120;
    }
  }

  foreach($rows as $rowKey => $rowValue)
  {
      unset($rows[$rowKey]->actions);
  }
}
      ...
      <?php if(empty($stories)):?>
        <div class="table-empty-tip">
          <p>
            ...
            <?php if(common::canModify('product', $product) and common::hasPriv('story', 'create') and $browseType == 'allstory' and $canBeChangedByProject):?>
