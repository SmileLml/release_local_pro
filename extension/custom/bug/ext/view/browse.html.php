...
if($allNoCheckbox) $cols[0]->checkbox = false;

$canBeChangedByProject = true;
if($product->shadow && empty($config->CRProject) and $product->status == 'closed') $canBeChangedByProject = false;
...
js::set('projectModels',  $projectModels);
js::set('showExecution',  $showExecution);
js::set('bugExecution',   $projectModel == 'kanban' ? $lang->bug->kanban : $lang->bug->execution);
...
<div id="mainMenu" class="clearfix">
  ...
  <?php if(!isonlybody()):?>
    <div class="btn-toolbar pull-right">
      ...
      <?php if(common::canModify('product', $product) and $canBeChangedByProject):?>
        <?php
        $createBugLink   = '';
        $batchCreateLink = '';
    ...
    <?php if(empty($bugs)):?>
      <div class="table-empty-tip">
        <p>
          ...
          <?php if(common::canModify('product', $product) and common::hasPriv('bug', 'create') and $canBeChangedByProject):?>
...
<div class="modal fade" id="adjustModal" tabindex="-1" role="dialog">
  <div class="modal-dialog w-860px">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal"><i class="icon icon-close"></i></button>
