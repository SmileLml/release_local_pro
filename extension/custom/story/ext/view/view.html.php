...
<?php
$canBeChange = true;
if(in_array($from, array('project', 'execution')))
{
  $canBeChange = common::canModify($from, $project);
  if(isset($project) && isset($project->multiple) && !$project->multiple && $from = 'project') $canBeChange = common::canModify('execution', $project);
  if(!empty($product->shadow && isset($projectInfo))) $canBeChange = common::canModify('project', $projectInfo);
}
?>
    ...
    <?php if(!isonlybody()):?>
      <div class="btn-toolbar pull-right">
        <?php if(common::canModify('product', $product)): ?>
          ...
          ?>
          <?php if($canBeChange) common::printLink('story', 'create', "productID={$story->product}&branch={$story->branch}&moduleID={$story->module}&$otherParam&bugID=0&planID=0&todoID=0&extra=&storyType=$story->type", "<i class='icon icon-plus'></i> " . $lang->story->create, '', "class='btn btn-primary' data-app='$tab'"); ?>
        <?php endif;?>
      </div>
    <?php endif;?>
  </div>
            ...
            <?php if(!empty($twins)):?>
              <div class='tab-pane active' id='legendTwins'>
                <ul class="list-unstyled">
                  <?php include $app->getModulePath() .  'view/blocktwins.html.php';?>
                </ul>
              </div>
            <?php endif;?>
            <?php if($this->config->URAndSR and !$hiddenURS and $config->vision != 'or'):?>
                ...
                <ul class="list-unstyled">
                  <?php
                    ...
                    foreach($relation as $id => $title)
                    {
                      ...
                      echo $canBeChange ? html::a($this->createLink('story', 'linkStory', "storyID=$story->id&type=remove&linkedID=$id&browseType=&queryID=0&storyType=$story->type"), '<i class="icon icon-unlink btn-info"></i>', 'hiddenwin', "class='hide removeButton'") : '';
                    }
                  ?>
                  <?php $linkLang = ($story->type == 'story') ? $lang->story->requirement : $lang->story->story;?>
                  <li><?php if(common::hasPriv($story->type, 'linkStory') and $canBeChange) echo html::a($this->createLink('story', 'linkStory', "storyID=$story->id&type=linkStories&linkedID=0&browseType=&queryID=0&storyType=$story->type", '', true), $lang->story->link . $linkLang, '', "class='btn btn-info iframe' data-width='95%' id='linkButton'");?>
                </ul>
              </div>
            <?php endif;?>
