...
$canCreateRegion    = common::hasPriv('kanban', 'createRegion') && $groupBy == 'default' && $canBeChangedByProject;
$canEditRegion      = commonModel::hasPriv('kanban', 'editRegion') && $canBeChangedByProject;
$canDeleteRegion    = commonModel::hasPriv('kanban', 'deleteRegion') && $canBeChangedByProject;
$canCreateLane      = commonModel::hasPriv('kanban', 'createLane') && $canBeChangedByProject;
$canCreateTask      = common::hasPriv('task', 'create') && $canBeChangedByProject;
$canBatchCreateTask = common::hasPriv('task', 'batchCreate') && $canBeChangedByProject;
$canImportTask      = common::hasPriv('execution', 'importTask') && $execution->multiple && $canBeChangedByProject;

$canCreateBug        = $features['qa'] && $productID && common::hasPriv('bug', 'create') && $canBeChangedByProject;
$canBatchCreateBug   = $features['qa'] && $productID && common::hasPriv('bug', 'batchCreate') && $execution->multiple && $canBeChangedByProject;
$canImportBug        = $features['qa'] && $productID && common::hasPriv('execution', 'importBug') && $execution->multiple && $canBeChangedByProject;
$hasBugButton        = $features['qa'] && ($canCreateBug || $canBatchCreateBug) && $canBeChangedByProject;

$canCreateStory      = $features['story'] && $productID && common::hasPriv('story', 'create') && $canBeChangedByProject;
$canBatchCreateStory = $features['story'] && $productID && common::hasPriv('story', 'batchCreate') && $canBeChangedByProject;
$canLinkStory        = $features['story'] && $productID && common::hasPriv('execution', 'linkStory') && $canBeChangedByProject;
$canLinkStoryByPlan  = $features['story'] && $productID && common::hasPriv('execution', 'importplanstories') && !empty($project->hasProduct) && $canBeChangedByProject;
$hasStoryButton      = $features['story'] && ($canCreateStory || $canBatchCreateStory || $canLinkStory || $canLinkStoryByPlan) && $canBeChangedByProject;

$hasTaskButton = $canCreateTask || $canBatchCreateTask || $canImportBug;

js::set('priv',
  array(
    'canCreateTask'         => $canCreateTask && $canBeChangedByProject,
    'canBatchCreateTask'    => $canBatchCreateTask && $canBeChangedByProject,
    'canImportBug'          => $canImportBug && $canBeChangedByProject,
    'canCreateBug'          => $canCreateBug && $canBeChangedByProject,
    'canBatchCreateBug'     => $canBatchCreateBug && $canBeChangedByProject,
    'canCreateStory'        => $canCreateStory && $canBeChangedByProject,
    'canBatchCreateStory'   => $canBatchCreateStory && $canBeChangedByProject,
    'canLinkStory'          => $canLinkStory && $canBeChangedByProject,
    'canLinkStoryByPlan'    => $canLinkStoryByPlan && $canBeChangedByProject,
    'canViewBug'            => common::hasPriv('bug', 'view'),
    'canAssignBug'          => common::hasPriv('bug', 'assignto') && $canBeChangedByProject,
    'canConfirmBug'         => common::hasPriv('bug', 'confirmBug') && $canBeChangedByProject,
    'canResolveBug'         => common::hasPriv('bug', 'resolve') && $canBeChangedByProject,
    'canCopyBug'            => common::hasPriv('bug', 'create') && $canBeChangedByProject,
    'canEditBug'            => common::hasPriv('bug', 'edit') && $canBeChangedByProject,
    'canDeleteBug'          => common::hasPriv('bug', 'delete') && $canBeChangedByProject,
    'canActivateBug'        => common::hasPriv('bug', 'activate') && $canBeChangedByProject,
    'canViewTask'           => common::hasPriv('task', 'view'),
    'canAssignTask'         => common::hasPriv('task', 'assignto') && $canBeChangedByProject,
    'canFinishTask'         => common::hasPriv('task', 'finish') && $canBeChangedByProject,
    'canPauseTask'          => common::hasPriv('task', 'pause') && $canBeChangedByProject,
    'canCancelTask'         => common::hasPriv('task', 'cancel') && $canBeChangedByProject,
    'canCloseTask'          => common::hasPriv('task', 'close') && $canBeChangedByProject,
    'canActivateTask'       => common::hasPriv('task', 'activate') && $canBeChangedByProject,
    'canActivateStory'      => common::hasPriv('story', 'activate') && $canBeChangedByProject,
    'canStartTask'          => common::hasPriv('task', 'start') && $canBeChangedByProject,
    'canRestartTask'        => common::hasPriv('task', 'restart') && $canBeChangedByProject,
    'canEditTask'           => common::hasPriv('task', 'edit') && $canBeChangedByProject,
    'canDeleteTask'         => common::hasPriv('task', 'delete') && $canBeChangedByProject,
    'canRecordEstimateTask' => common::hasPriv('task', 'recordEstimate') && $canBeChangedByProject,
    'canToStoryBug'         => common::hasPriv('story', 'create') && $canBeChangedByProject,
    'canAssignStory'        => common::hasPriv('story', 'assignto') && $canBeChangedByProject,
    'canEditStory'          => common::hasPriv('story', 'edit') && $canBeChangedByProject,
    'canDeleteStory'        => common::hasPriv('story', 'delete') && $canBeChangedByProject,
    'canChangeStory'        => common::hasPriv('story', 'change') && $canBeChangedByProject,
    'canCloseStory'         => common::hasPriv('story', 'close') && $canBeChangedByProject,
    'canUnlinkStory'        => (common::hasPriv('execution', 'unlinkStory') && !empty($execution->hasProduct)) && $canBeChangedByProject,
...
<div id='mainMenu' class='clearfix'>
  ...
  <div class='btn-toolbar pull-right'>
    ...
    if(!empty($editURL) and $canBeChangedByProject) echo html::a($editURL, '<i class="icon icon-edit"></i> ' . $lang->edit, '', "class='iframe btn btn-link text-left' data-width='80%'");
    $actions           = '';
    $printSettingBtn   = (common::hasPriv('execution', 'close') or common::hasPriv('execution', 'delete') or !empty($executionActions));

    if($printSettingBtn and $execution->multiple and $canBeChangedByProject)
    {
    ...
    echo $actions;
    ?>
    <?php if($canBeChangedByProject and ($canCreateTask or $canBatchCreateTask or $canImportBug or $canCreateBug or $canBatchCreateBug or $canCreateStory or $canBatchCreateStory or $canLinkStory or $canLinkStoryByPlan)):?>
          ...
          <div class="region<?php if($canSortRegion) echo ' sort';?>" data-id="<?php echo $region->id;?>">
            <div class="region-header dropdown">
              ...
              <div class='region-actions'>
                <?php if(($canEditRegion || $canCreateLane || $canDeleteRegion || $canCreateRegion) && $canBeChangedByProject):?>
