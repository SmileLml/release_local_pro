...
$allNoCheckboxCount = 0;
foreach($executions as $executionChild) if(isset($executionChild->projectStatus) && $executionChild->projectStatus == 'closed' && isset($config->CRProject) && empty($config->CRProject)) $allNoCheckboxCount++;
if($allNoCheckboxCount == count($executions)) $cols[0]->checkbox = false;
...
js::set('CRProject', isset($config->CRProject) && empty($config->CRProject) ? 1 : 0);
?>
...
<div id='mainContent' class="main-row fade">
  ...
  const options =
  {
    ...
    canRowCheckable: function(rowID)
    {
      if(this.layout.rowsMap[rowID] == undefined) return false;
      if(typeof this.layout.rowsMap[rowID] != undefined && typeof this.layout.rowsMap[rowID].data != undefined && this.layout.rowsMap[rowID].data.projectStatus != undefined && this.layout.rowsMap[rowID].data.projectStatus == 'closed' && CRProject == 1) return false;
      return true;
    }
  };
