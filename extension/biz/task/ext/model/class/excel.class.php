...
class excelTask extends taskModel
{
    ...
    public function createFromImport($executionID)
    {
        ...
        foreach($data->execution as $key => $execution)
        {
            ...
            if(!empty($this->config->limitTaskDate)) $this->checkEstStartedAndDeadline($execution, $taskData->estStarted, $taskData->deadline);
        ...
        foreach($tasks as $key => $taskData)
        {
            ...
            else
            {
                ...
                {
                    if($this->session->lastParentID and !isset($parentID)) $parentID = $this->session->lastParentID;
                ...
                if(!dao::isError())
                {
                    ...
                    {
                        $parentID = $taskID;
                        $this->session->set('lastParentID', $parentID);
                    }
    ...
    public function processDatas4Task($taskData)
    {
        ...
        foreach($datas as $key => $data)
        {
            ...
            if(strpos($data->name, '>') !== 0 and isset($data->estimate))
            {
                ...
                $this->session->set('lastParentRow'   , $key);
                $this->session->set('lastEstimateList', $estimateList);
            }
            if(strpos($data->name, '>') === 0 and isset($data->estimate))
            {
                $parentRow        = $this->session->lastParentRow;
                $lastEstimateList = $this->session->lastEstimateList;

                if(!isset($parentList[$parentRow]))   $parentList[$parentRow]   = $parentRow;
                if(!isset($estimateList[$parentRow])) $estimateList[$parentRow] = $lastEstimateList[$parentRow];
