...
class trip extends control
{
    ...
    {
        die($this->fetch('trip', 'browse', "mode=personal&date=$date&orderBy=$orderBy", ''));
    }
    ...
    {
        die($this->fetch('trip', 'browse', "mode=department&date=$date&orderBy=$orderBy", ''));
    }
    ...
    {
        die($this->fetch('trip', 'browse', "mode=company&date=$date&orderBy=$orderBy", ''));
    }
        ...
        if($_POST)
        {
            ...
            if($this->post->customers)
            {
                ...
                if($customers)
                {
                    ...
                    {
                        $this->action->create($customer->relation == 'provider' ? 'provider' : 'customer', $customer->id, "create{$this->type}", '', baseHTML::a(inlink('view', "tripID={$tripID}"), $this->post->name, "data-toggle='modal'"));
                    }
                }
            }
    ...
    public function edit($id, $customers = '')
    {
        ...
        {
            $locate = helper::safe64Encode(helper::createLink("{$this->type}", 'browse'));
        ...
        if($_POST)
        {
            ...
            if($result)
            {
                $actionID = $this->loadModel('action')->create($trip->type, $id, 'edited');
