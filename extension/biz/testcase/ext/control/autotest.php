...
class autotest
{
    const ztURL        = 'http://10.42.41.95:8000';            // ZenTaoPMS deploys domain names.
     ...
     * @return string
     */
    public function getleft($optionalParams = array(), $extraFields = array())
    {
        $result = $this->setParams(array('m' => 'api', 'f' => 'zentaolefttime'), $optionalParams)
            ->setRequestMethod('get')
            ->sendRequest(array('get' => self::ztURL), true);

        $extraFields  = array_merge(array('lefttime'), $extraFields);
        $returnResult = $this->getResultData($result, $extraFields);
        return $returnResult;
    }
    /**
     ...
     * @return string
     */
    public function Edittestcase($testcaseID=0,$optionalParams = array())
    {
        $result = $this->setParams(array(), $optionalParams)
            ->setRequestMethod('post')
            ->sendRequest(array('get' => self::ztURL . '/api/zentaocase/'.$testcaseID, 'path_info' => self::ztURL . '/api/zentaocase/'.$testcaseID));
        $returnResult = $this->returnResult;
        if (!empty($result->message) && strpos($result->message, 'success')) $returnResult = array('status' => 1, 'msg' => 'success', 'result' => $result->message);
        else if(!empty($result->message))$returnResult = array('status' => 0, 'msg' => 'fail', 'result' => $result->message);
        else $returnResult = array('status' => 0, 'msg' => 'fail', 'result' => '无法连接测试部自动化平台请检查服务是否关闭');
        return $returnResult;    
}
    /**
    ...
    public function addDept($optionalParams = array())
    {
        $result = $this->setParams(array(), $optionalParams)
            ->setRequestMethod('post')
            ->sendRequest(array('get' => self::ztURL . '/dept-manageChild.json', 'path_info' => self::ztURL . '/dept-manageChild.json'));
     ...
     * @return string
     */
    public function addOwner($optionalParams = array())
    {
        $result = $this->setParams(array(), $optionalParams)
            ->setRequestMethod('post')
            ->sendRequest(array('get' => self::ztURL . '/outer_bug/case_module_owner', 'path_info' => self::ztURL . '/outer_bug/case_module_owner'));
        $returnResult = $this->returnResult;
        if (!empty($result->message) && strpos($result->message, 'success')) $returnResult = array('status' => 1, 'msg' => 'success', 'result' => $result->message);
        else if(!empty($result->message))$returnResult = array('status' => 0, 'msg' => 'fail', 'result' => $result->message);
        else $returnResult = array('status' => 0, 'msg' => 'fail', 'result' => '无法连接测试部自动化平台请检查服务是否关闭');
    ...
    {
        $returnResult = $this->returnResult;
        if (!empty($responseData->message) && strcmp($responseData->message, 'success') === 0)
        {
            $returnResult['result'] = array('status' => 1, 'msg' => 'success');
        ...
        elseif (!empty($responseData->result))
        {
            ...
            $returnResult['result'] = array('status' => 0, 'msg' => "无法连接测试部自动化平台请检查服务是否关闭");
        }
        //return json_encode($returnResult['result'],JSON_UNESCAPED_UNICODE);
