...
class user extends control
{
	 ...
	 */
        ...
        if(!empty($_POST) or (isset($_GET['account']) and isset($_GET['password'])))
        {
            ...
            /* realm */
            if($this->post->realm) $realm = $this->post->realm;
            setcookie('realm', $realm, $this->config->cookieLife, $this->config->webRoot);
            ...
            $user = $this->user->identify($account, $password, $realm);
	    ...
	    $this->view->realm     = $this->cookie->realm;
            if ($this->view->realm == null) $this->view->realm = $this->config->user->defaultRealm;
            $this->display();
        }
    }
