...
class userModel extends model
{
     ...
     * @return  object
     */
    public function identify($account, $password, $realm='kydc')
    {
        ...
        /* kydc login */
        if($realm=='kydc'){
               $email = exec($this->app->getAppRoot()."bin/oauth_login.py ". escapeshellarg($account) . " " . escapeshellarg($password));
               if ($email=='') return false;
               $record = $this->dao->select('*')->from(TABLE_USER)
                   ->where('email')->eq($email)
                   ->andwhere('deleted')->eq(0)
                   ->fetch();
               $user = $record ? $record : false;
        } else { /* legacy login */
