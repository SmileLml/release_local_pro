...
class projectModel extends model
{
    ...
    {
        $stmt = $this->dao->select('t1.id,t1.estimate,t1.`left`,t1.consumed,t1.status')->from(TABLE_TASK)->alias('t1')
            ->leftJoin(TABLE_PROJECT)->alias('t2')->on('t1.execution = t2.id')
	...
	while($task = $stmt->fetch())
	{
		...
		$totalLeft     += $task->left;
		if(in_array($task->status, array('closed', 'cancel'))) $closedTotalLeft += $task->left;
	}

        $workhour = new stdclass();
        $workhour->totalEstimate = $totalEstimate;
        $workhour->totalConsumed = $totalConsumed;
        $workhour->totalLeft     = round($totalLeft - $closedTotalLeft, 1);
    ...
    public function getStatData($projectID)
    {
        ...
        $taskStmt      = $this->dao->select('id,status,finishedBy,deadline')->from(TABLE_TASK)->where('execution')->in(array_keys($executions))->andWhere('deleted')->eq(0)->query();
	$today 	       = helper::today();
	$taskCount     = 0;
	$finishedCount = 0;
	$delayedCount  = 0;
	...
	{
		if(!isset($statusPairs[$task->status])) $statusPairs[$task->status] = 0;

		$taskCount ++;
		$statusPairs[$task->status] ++;
		if(!empty($task->finishedBy)) $finishedCount ++;
		if(in_array($task->status, array('wait', 'doing')) && !helper::isZeroDate($task->deadline) && $task->deadline < $today) $delayedCount ++;
	}
    ...
    public function setMenu($objectID)
    {
        ...
        if(!empty($this->config->URAndSR) && $project->model !== 'kanban' && isset($lang->project->menu->storyGroup))
        {
            ...
            if(isset($project->model) && $project->model !== 'kanban' && isset($lang->project->menu->storyGroup))
            {
