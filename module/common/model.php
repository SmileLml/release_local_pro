   ...
   /**
    ...
    public static function printCreateList()
    {
        ...
        if($productID)
        {
            ...
            $product = $dao->select('id')->from(TABLE_PRODUCT)->where("`deleted` = '0' and vision = '{$config->vision}' and id = '{$productID}'")->fetch();
        ...
        if(!$productID and $app->user->view->products)
        {
            ...
            $product = $dao->select('id')->from(TABLE_PRODUCT)->where("`deleted` = '0' and vision = '{$config->vision}' and id " . helper::dbIN($app->user->view->products) . " order by `order` desc limit 1")->fetch();
            if($product) $productID = $product->id;
        }

        if($config->vision == 'lite')
        {
            $condition  = "`deleted` = '0' AND `vision` = 'lite' AND `model` = 'kanban'";
            if(!$app->user->admin) $condition .= " AND `id` " . helper::dbIN($app->user->view->projects);

            //$object = $app->dbQuery("select id from " . TABLE_PROJECT . ' WHERE ' . $condition . ' LIMIT 1')->fetch();
            $object = $dao->select("id")->from(TABLE_PROJECT)->where($condition)->limit(1)->fetch();
...
EOF;
    ...
    public static function createChanges($old, $new, $moduleName = '')
    {
        ...
        if($config->edition != 'open')
        {
            ...
            {
                $field = $dao->select('options')->from(TABLE_WORKFLOWFIELD)->where("`module` = '$moduleName' AND `field` = 'subStatus'")->fetch();
                if(!empty($field->options)) $field->options = json_decode($field->options, true);

                if(!empty($field->options[$newStatus]['default']))
                {
                    $flow    = $dao->select('`table`')->from(TABLE_WORKFLOW)->where("`module`='$moduleName'")->fetch();
            ...
            $dateFields = array();
            $result = $dao->select('`field`')->from(TABLE_WORKFLOWFIELD)->where("`module` = '{$moduleName}' and `control` in ('date', 'datetime')")->fetchAll();
            foreach($result as $row) $dateFields[$row->field] = $row->field;
        }
    ...
    public static function hasPriv($module, $method, $object = null, $vars = '')
    {
        ...
        /* Disable toolBar bulk pperations. */
        if($module == 'project' and $method == 'batchedit' and empty($config->CRProject) and $app->session->currentProjectsIsColse) return false;
     ...
     * @return bool
     */
    public static function canModify($type, $object, $parentObject = null)
    {
        global $config;

        if(empty($object)) return true;

        if($type == 'project'   and isset($config->CRProject)    and empty($config->CRProject) and $object->status == 'closed') return false;
        if($type == 'product'   and empty($config->CRProduct)    and $object->status == 'closed') return false;
        if($type == 'execution' and (empty($config->CRExecution) and $object->status == 'closed') or (isset($config->CRProject) and empty($config->CRProject) and isset($parentObject) and $parentObject->status == 'closed')) return false;
        if($type == 'execution' and isset($config->CRProject)    and empty($config->CRProject) and is_null($parentObject) and isset($object->project))
        {
            $modelClass = class_exists("extcommonModel") ? "extcommonModel" : "commonModel";
            $project = (new $modelClass)->loadModel('project')->getByID($object->project);
            if(isset($project) and isset($project->status) and $project->status  == 'closed') return false;
        }
        if($type == 'projectproduct' and isset($config->CRProject) and empty($config->CRProject) and isset($object->shadow) and $object->shadow and $object->status == 'closed') return false;
        return true;
    }
