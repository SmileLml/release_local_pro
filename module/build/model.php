...
class buildModel extends model
{
    ...
    public function getPairs($products = 'all', $projects = 'all')
    {
            return $this->dao->select('id, name')->from(TABLE_BUILD)
		    ->where('1=1')
                    ->beginIF($products and $products != 'all')->andWhere('product')->in($products)->fi()
                    ->beginIF($projects and $projects != 'all')->andWhere('project')->in($projects)->fi()
		    ->fetchPairs();
    }
