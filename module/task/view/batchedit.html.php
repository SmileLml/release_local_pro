      ...
      <table class='table table-form table-fixed with-border'>
        <thead>
          <tr>
            ...
            <th class='required <?php if((count($visibleFields) + count($extendFields)) > 10) echo 'c-name';?>'><?php echo $lang->task->name?></th>
            ...
            {
                    if($extendField->control == 'select' && count($extendField->options) > $this->config->maxCount) $options[$extendField->field] = $extendField->options;
                    echo "<th class='c-extend'>{$extendField->name}</th>";
            }
            ?>
          </tr>
        </thead>
        <tbody>
	  <?php $moduleGroup = array();?>
          <?php foreach($taskIDList as $taskID):?>
          <?php
          $task = $tasks[$taskID];
          if(!isset($execution))
          {
		  if(!isset($moduleGroup[$task->execution]))
		  {
			  ...
			  $moduleGroup[$task->execution] = $modules;
		  }
		  $modules = $moduleGroup[$task->execution];
          }
          ?>
          <tr>
            ...
            $teamAccounts = !empty($executionTeams[$task->execution]) ? array_keys($executionTeams[$task->execution]) : array();
            ...
            <td style='overflow:visible' title='<?php echo $task->name?>'>
              <div class="input-control has-icon-right">
                <?php echo html::input("names[$taskID]", $task->name, "class='form-control'");?>
                <div class="colorpicker">
                  <button type="button" class="btn btn-link dropdown-toggle" data-toggle="dropdown"><span class="cp-title"></span><span class="color-bar"></span><i class="ic"></i></button>
                  <ul class="dropdown-menu clearfix">
                    <li class="heading"><?php echo $lang->story->colorTag;?><i class="icon icon-close"></i></li>
                  </ul>
                  <?php echo html::hidden("colors[$taskID]", $task->color, "data-provide='colorpicker' data-wrapper='input-control-icon-right' data-icon='color' data-btn-tip='{$lang->task->colorTag}' data-update-text='#names\\[{$taskID}\\]'");?>
                </div>
              </div>
            </td>
            <td class='text-left<?php echo zget($visibleFields, 'module', ' hidden')?>' style='overflow:visible'><?php echo html::select("modules[$taskID]", $modules, $task->module, "class='form-control picker-select' data-drop-width='auto'")?></td>
            <td class='text-left<?php echo zget($visibleFields, 'assignedTo', ' hidden')?>' style='overflow:visible'><?php echo html::select("assignedTos[$taskID]", $taskMembers, $task->assignedTo, "class='form-control picker-select' data-drop-width='auto' {$disableAssignedTo}");?></td>
            <td><?php echo html::select("types[$taskID]",    $typeList, $task->type, "class='form-control'");?></td>
            <td <?php echo zget($visibleFields, 'status',     "class='hidden'")?>><?php echo html::select("statuses[$taskID]", $statusList, $task->status, "class='form-control' {$disableHour}");?></td>
            <td <?php echo zget($visibleFields, 'estStarted', "class='hidden'")?>><?php echo html::input("estStarteds[$taskID]", helper::isZeroDate($task->estStarted) ? '' : $task->estStarted, "class='form-control text-center form-date'");?></td>
            <td <?php echo zget($visibleFields, 'deadline',   "class='hidden'")?>><?php echo html::input("deadlines[$taskID]", helper::isZeroDate($task->deadline) ? '' : $task->deadline, "class='form-control text-center form-date'");?></td>
            <td <?php echo zget($visibleFields, 'pri',        "class='hidden'")?>><?php echo html::select("pris[$taskID]", $priList, $task->pri, "class='form-control'");?></td>
            <td <?php echo zget($visibleFields, 'estimate',   "class='hidden'")?>><?php echo html::input("estimates[$taskID]", $task->estimate, "class='form-control text-center' {$disableHour}");?></td>
            ...
            <td <?php echo zget($visibleFields, 'left',       "class='hidden'")?>><?php echo html::input("lefts[$taskID]", $task->left, "class='form-control text-center' {$disableHour}");?></td>
            <td class='text-left<?php echo zget($visibleFields, 'finishedBy', ' hidden')?>' style='overflow:visible'><?php echo html::select("finishedBys[$taskID]", $members, $task->finishedBy, "class='form-control picker-select' data-drop-width='auto'");?></td>
            <td class='text-left<?php echo zget($visibleFields, 'canceledBy', ' hidden')?>' style='overflow:visible'><?php echo html::select("canceledBys[$taskID]", $members, $task->canceledBy, "class='form-control picker-select' data-drop-width='auto'");?></td>
            <td class='text-left<?php echo zget($visibleFields, 'closedBy', ' hidden')?>' style='overflow:visible'><?php echo html::select("closedBys[$taskID]",   $members, $task->closedBy, "class='form-control picker-select' data-drop-width='auto'");?></td>
            ...
            foreach($extendFields as $extendField)
            {
                if(isset($options[$extendField->field])) $extendField->options = array($task->{$extendField->field} => $options[$task->{$extendField->field}]);
                echo "<td" . (($extendField->control == 'select' or $extendField->control == 'multi-select') ? " style='overflow:visible'" : '') . ">" . $this->loadModel('flow')->getFieldControl($extendField, $task, $extendField->field . "[{$taskID}]") . "</td>";
            }
            ?>
          </tr>
          <?php endforeach;?>
...
<?php js::set('extOptions', $options);?>
<script>
$(function()
{
    <?php foreach(array_keys($options) as $fieldName):?>
    $('select[data-field="<?php echo $fieldName;?>"]').next().mousedown(function()
    {
        ...
        $select.empty();
        options = extOptions['<?php echo $fieldName?>'];
        optionsHtml = '';
        for(key in options)
        {
    	    value = options[key];
    	    optionsHtml += '<option value="' + key + '" title="' + value + '">' + value + '</option>';
        }
