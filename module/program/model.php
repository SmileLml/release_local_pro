...
class programModel extends model
{
     ...
     * @return bool
     */
    public static function isClickable($program, $action, $module)
    {
        ...
        if($module == 'project')
        {
            ...
            if($action == 'edit' and $module == 'project') return !empty($config->CRProject) or $program->status != 'closed';
            if($action == 'team' and $module == 'project') return !empty($config->CRProject) or $program->status != 'closed';

            if($action == 'whitelist')      return $program->acl != 'open' and (!empty($config->CRProject) or $program->status != 'closed');
            if($action == 'group')          return $program->model != 'kanban' and (!empty($config->CRProject) or $program->status != 'closed');
            if($action == 'manageproducts') return $program->model != 'kanban' and (!empty($config->CRProject) or $program->status != 'closed');
        }
