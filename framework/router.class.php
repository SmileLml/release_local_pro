    ...
    public function loadLang($moduleName, $appName = '')
    {
        ...
        if($moduleName == 'common' and $this->dbh and !empty($this->config->db->name))
        {
            ...
            try
            {
                ...
                //$customMenus = $dao->select('*')->from(TABLE_LANG)->where("`module`='common' AND `section`='mainNav' AND `lang`='{$this->clientLang}' AND `vision`='{$this->config->vision}'")->fetchAll();
            }
    ...
    public function setCommonLang()
    {
        ...
        if($this->dbh and !empty($this->config->db->name))
        {
            ...
            if(!defined('IN_UPGRADE'))
            {
                ...
                //$URSRList   = $dao->select('`key`, `value`')->from(TABLE_LANG)->where("`module` = 'custom' and `section` = 'URSRList' and `lang` = '{$clientLang}'")->fetchAll();
                ...
                //if(empty($URSRList)) $URSRList = $dao->select('`key`, `value`')->from(TABLE_LANG)->where("module = 'custom' and `section` = 'URSRList' and `key` = '{$config->URSR}'")->fetchAll();
            ...
            try
            {
                ...
                //$customMenus = $dao->select('*')->from(TABLE_LANG)->where("`module`='common' AND `lang`='{$this->clientLang}' AND `section`='' AND `vision`='{$config->vision}'")->fetchAll();
            }
            catch(PDOException $exception){}
