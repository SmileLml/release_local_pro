...
class baseRouter
{
    ...
    public function connectToRedis()
    {
        ...
        try
        {
            ...
            {
                $redis->pconnect($config->redis->host , $config->redis->port, $config->redis->timeout, '', 0, 0, ['auth' => [$config->redis->username, $config->redis->password]]);
            }
            else
            {
                $redis->pconnect($config->redis->host , $config->redis->port, $config->redis->timeout, '', 0, 0);
   ...
   /**
    ...
    public function shutdown()
    {
	    ...
	    if(isset($this->redis) && $this->redis) $this->redis->close();
